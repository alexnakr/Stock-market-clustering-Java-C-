...
public class KMeansReducer extends
	...
	protected void reduce(ClusterCenter key, Iterable<Vector> values,
		...
		newCenter = new Vector("cen",vectorSize);
		for (Vector value : values) 
		{
			vectorList.add(new Vector(value));
			int len = Math.min(value.getVector().length, newCenter.vector.length);
			for (int i = 0; i < len; i++) 
			{
				for(int j = 0 ; j < 4; j++)
				   newCenter.getVector()[i].data[j] += value.getVector()[i].data[j];
			}
		}
		...
		{
		    for(int j = 0; j < 4 ; j++)
			   newCenter.getVector()[i].data[j] = newCenter.getVector()[i].data[j]/vectorList.size();
		}
